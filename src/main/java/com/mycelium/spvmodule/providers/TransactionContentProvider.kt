        ...
        when (match) {
            TRANSACTION_SUMMARY_ID ->
                if (selection == TransactionSummary.SELECTION_ACCOUNT_INDEX) {
                    ...
                    val accountId = selectionArgs!!.get(0)

                    val transactionsSummary = service.getTransactionsSummary(accountId)
            ...
            TRANSACTION_DETAILS_ID ->
                if (selection == TransactionDetails.SELECTION_ACCOUNT_INDEX) {
                    ...
                    val transactionDetails = service.getTransactionDetails(accountId, hash) ?: return cursor
            ...
            ACCOUNT_BALANCE_ID, ACCOUNT_BALANCE_LIST -> {
                ...
                if (selection == AccountBalance.SELECTION_ACCOUNT_INDEX) {
                    ...
                    listOf(selectionArgs!!.get(0)).forEach { accountId ->
                        val columnValues = listOf(
                                accountId,                             //TransactionContract.AccountBalance._ID
                                service.getAccountBalance(accountId),  //TransactionContract.AccountBalance.CONFIRMED
                                service.getAccountSending(accountId),  //TransactionContract.AccountBalance.SENDING
                                service.getAccountReceiving(accountId) //TransactionContract.AccountBalance.RECEIVING
                        )
                        cursor.addRow(columnValues)
                    }
                    ...
                    service.getAccountIndices().forEach { accountId ->
                        val columnValues = listOf(
                                accountId,                             //TransactionContract.AccountBalance._ID
                                service.getAccountBalance(accountId),  //TransactionContract.AccountBalance.CONFIRMED
                                service.getAccountSending(accountId),  //TransactionContract.AccountBalance.SENDING
            ...
            CALCULATE_MAX_SPENDABLE_ID -> {
                ...
                if (selection == CalculateMaxSpendable.SELECTION_COMPLETE) {
                    val accountId = selectionArgs!![0]
                    val txFeeStr = selectionArgs[1]
                    val txFee = TransactionFee.valueOf(txFeeStr)
                    val txFeeFactor = selectionArgs[2].toFloat()
                    val maxSpendableAmount = service.calculateMaxSpendableAmount(accountId, txFee, txFeeFactor)
                    val columnValues = listOf(
            ...
            CHECK_SEND_AMOUNT_ID -> {
                ...
                if (selection == CheckSendAmount.SELECTION_COMPLETE) {
                    ...
                    val checkSendAmount = service.checkSendAmount(accountId, txFee, txFeeFactor, amountToSend)
                    val columnValues = listOf(
            ...
            GET_PRIVATE_KEYS_COUNT_ID -> {
                ...
                cursor.addRow(listOf(Bip44AccountIdleService.getInstance()!!.getPrivateKeysCount(accountId)))
            }
            else -> {
                // Do nothing.
            }
